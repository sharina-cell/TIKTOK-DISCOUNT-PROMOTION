...
3. **Working sheet** - merges the report + Masterfile on SKU, adds:
   ...
   - `remarks` = "no discount remarks" when every tier's discount computes to 0%
4. **RRP MISMATCH tab** - every row where RRP CHECK is False, pulled out for review.
5. **TO UPLOAD `<tier>` tabs** - one per tier, in that marketplace's official
   upload template layout. Only rows with a valid price for that tier are
   included.
...
- The "no discount remarks" rule currently fires only when *all* configured
  tiers compute to exactly 0% for a row. If you want per-tier remarks
  instead, say so and it's a small tweak to `build_working_sheet()`.
