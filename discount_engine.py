...
def parse_exclusion_skus(manual_text: str = "", file_bytes: bytes = None, file_name: str = "") -> set:
    """
    Builds a set of cleaned SKUs to exclude from the promo, from either/both:
      - manual_text: free text, one or more SKUs separated by commas, spaces,
        semicolons, or newlines
      - file_bytes: an uploaded .xlsx or .csv. If a column header containing
        "sku" is found, that column is used; otherwise the first column.
    """
    skus = set()

    if manual_text:
        for part in re.split(r"[,\n\r\t;]+", manual_text):
            cleaned = _clean_sku(part)
            if cleaned:
                skus.add(cleaned)

    if file_bytes:
        is_csv = file_name.lower().endswith(".csv") if file_name else False
        if not is_csv:
            try:
                ...
                for c in range(1, ws.max_column + 1):
                    ...
                    if header_val and "sku" in str(header_val).strip().lower():
                        col_idx = c
                        start_row = 2
                        break
                for r in range(start_row, ws.max_row + 1):
                    cleaned = _clean_sku(ws.cell(r, col_idx).value)
                    if cleaned:
                        skus.add(cleaned)
            except Exception:
                is_csv = True  # fall through to CSV parsing below
        ...
        if is_csv:
            ...
            start_row = 0
            if reader and reader[0] and "sku" in str(reader[0][0]).strip().lower():
                start_row = 1
            for row in reader[start_row:]:
                if row:
                    cleaned = _clean_sku(row[0])
                    if cleaned:
                        skus.add(cleaned)
...
def build_working_sheet(report_df: pd.DataFrame, master_df: pd.DataFrame, tier_names: list,
                         platform: str, excluded_skus: set = None) -> pd.DataFrame:
    """
    Left-join the report with master on SKU_MATCH. Computes:
      ...
      - EXCLUDED: True if SKU_MATCH is in excluded_skus
      - remarks: "excluded from promo" if excluded, else "no discount remarks"
        if all available tier discounts are 0
    """
    spec = PLATFORM_SPECS[platform]
    ...
    df["EXCLUDED"] = df["SKU_MATCH"].apply(lambda v: _clean_sku(v) in excluded_skus)
...
def format_for_display(working_df: pd.DataFrame, tier_names: list) -> pd.DataFrame:
    """
    Replaces blank/NaN values with the literal string "#N/A" in the columns
    that come from the Masterfile match (RRP, RRP CHECK, each tier's price,
    each tier's discount) - matching how unmatched products (bundles, GWP
    items, etc.) are shown in the original working files. Must only be
    applied right before writing/displaying - build_upload_sheet and
    build_mismatch_sheet rely on real NaN values to filter correctly, so run
    this on a copy, after those have already run.
    """
    cols = tier_names + ["RRP", "RRP CHECK"] + [f"{t} DISCOUNT" for t in tier_names]
    cols = [c for c in cols if c in working_df.columns]
    df = working_df.copy()
    for c in cols:
        df[c] = df[c].apply(lambda v: "#N/A" if pd.isna(v) else v)
    return df
...
def build_upload_sheet(working_df: pd.DataFrame, tier: str, platform: str) -> pd.DataFrame:
    ...
    (non-NA) price for that tier, a positive discount for that tier (0% or
    negative discounts are excluded), and are not flagged EXCLUDED.
    """
    spec = PLATFORM_SPECS[platform]
    disc_col = f"{tier} DISCOUNT"
...
def write_output_workbook(working_df: pd.DataFrame, mismatch_df: pd.DataFrame,
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        format_for_display(working_df, tier_names).to_excel(writer, sheet_name="Working Sheet", index=False)
        format_for_display(mismatch_df, tier_names).to_excel(writer, sheet_name="RRP MISMATCH", index=False)
        for tier in tier_names:
